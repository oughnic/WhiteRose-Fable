...
### Decided: iframe the published page
...
So `concept-index.json` was built for it: one document of 172 concepts in a single 64 KB fetch,
carrying each concept's slug, label, URL, plain-text definition, clause, the names it goes by now,
and — under `formerly` — the names it used to go by. It ships at
`input-model/concept-index.json` in the publication repo, generated by the mauro-edit
`concept-index` exporter from the same edition 1 decisions the redirects use, so the index and
the redirect table cannot disagree about what a concept used to be called.

It serves three places at once: type-ahead here, the hospital's directory boards and porter, and
a better concept lookup for contsys.org itself than the WebHelp index. Fetching 516 per-concept
fragments to build the same thing client-side was never an option.
...
- **WebXR.** CSS3D does not render in an immersive session, so this route forecloses VR. That
  was asked and answered before building: VR is out of scope. Were it ever back in, the answer
  would be text rendered into WebGL (`troika-three-text`) — crisp and VR-safe, but not real
  HTML, so not clickable, not searchable, not accessible.
...
4. The `/` search overlay over `concept-index.json`. Match `terms` and `formerly` and the
   definition text; a hit on `formerly` says so — "clinical process, now continuity of care
   process" — rather than silently redirecting, because the point of the room is to teach the
   vocabulary, not to paper over the fact that it changed.
...
- **Iframe the published page.** The screen shows the publication itself, not a copy of it.
- **`concept-index.json` is funded**, and built: mauro-edit `concept-index`, 172 concepts in one
  64 KB fetch, carrying definitions and former names as well as current ones. It ships in the
  publication repo at `input-model/concept-index.json`.
- **VR is out of scope.** `CSS3DRenderer` is therefore safe to build on, and the WebGL
  text-rendering alternative is not needed.
